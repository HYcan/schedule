...
import com.ronhe.romp.schedule.core.scheduler.cron.CronExpression;
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        if (conn==null || conn.isClosed()) {
                            conn = SchedulerResourceConfig.getAdminConfig().getDataSource().getConnection();
                        }
                        ...
                        //List<ScheduleJobInfoModel> scheduleList = XxlJobAdminConfig.getAdminConfig().getXxlJobInfoDao().scheduleJobQuery(nowTime + PRE_READ_MS);
                        List<ScheduleJobModel> scheduleList = null;
                        if (scheduleList!=null && scheduleList.size()>0) {
                            // 2、推送时间轮
                            for (ScheduleJobModel jobInfo: scheduleList) {
                            ...
                            // 3、更新trigger信息
                            for (ScheduleJobModel jobInfo: scheduleList) {
                                //SchedulerResourceConfig.getAdminConfig().getXxlJobInfoDao().scheduleUpdate(jobInfo);
                                SchedulerResourceConfig.getAdminConfig().getScheduleJobService().update(jobInfo);
                            }
